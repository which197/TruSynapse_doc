...
通用处理核心采取指令乱序超标量结构，实现了SIMD和矩阵运算，支持高性能数值计算。类脑功能部件采用了融合存内计算、近存计算和共享内存等多种关键技术，支持脑启发神经网络，实现连接计算。
...
基础核心软件支持通用并行计算环境，实现多核、多处理器和多机三层并行计算。系统支撑软件为应用提供服务支撑，TruSynapse是系统支撑软件之一。 
